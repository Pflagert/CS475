...
int main( int argc, char* argv[] ){
 ...
 // fprintf(stderr, "Process[%d]\n\tlowerBound_x = %d\tupperBound_x = %d\n\tlowerBound_y = %d\tupperBound_y = %d\n\ttile_x = %d\ttile_y = %d\n\ttile_len_y=%d\n",rank,lowerBound_x,upperBound_x,lowerBound_y, upperBound_y, tile_x, tile_y,tile_len_y);
  ...
  for( t = 0; t < cmdLineArgs.T; ++t ){
    ...
    if ( p_count != 1) {
        MPI_Recv(&space[read][lowerBound_y-1][lowerBound_x-1], tile_len_x, MPI_DOUBLE, left, tag, MPI_COMM_WORLD,&status);
    ...
    for( y = lowerBound_y; y < upperBound_y-1; ++y ){
      for( x = lowerBound_x; x < upperBound_x; ++x ){
          if ( y == 6 && x == 1 && rank == 0) {
                printf( "Before: space[%d][%d][%d] = %f\n\
                            space[%d][%d][%d] = %f\n\
                            space[%d][%d][%d] = %f\n\
                            space[%d][%d][%d] = %f\n\
                            space[%d][%d][%d] = %f\n\
    ...
    if ( p_count != 1){
        MPI_Recv(&space[read][upperBound_y-1][lowerBound_x-1], tile_len_x, MPI_DOUBLE, right, tag, MPI_COMM_WORLD,&status);
        for( x = lowerBound_x; x < upperBound_x; ++x )
            STENCIL( read, write, upperBound_y-1, x);
        MPI_Send(&space[write][upperBound_y-1][0], tile_len_x, MPI_DOUBLE, right, tag, MPI_COMM_WORLD);
    }
  // you may find this useful for debugging
   /*for( y = lowerBound_y-1; y <= upperBound_y; ++y ){
    for( x = lowerBound_x-1; x <= upperBound_x; ++x ){
