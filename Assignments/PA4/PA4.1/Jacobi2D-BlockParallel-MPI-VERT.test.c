...
int main( int argc, char* argv[] ){
       ...
       cmdLineArgs.problemSize*cmdLineArgs.problemSize )||(cmdLineArgs.tile_len_x != cmdLineArgs.problemSize) || (ceild(cmdLineArgs.problemSize,cmdLineArgs.tile_len_y) != p_count)){
	       if(rank == 0){
		         //fprintf(stderr,"The tile size is too small to accomodate the problem\n");
                   fprintf(stderr,"cmdLineArgs.tile_len_x*cmdLineArgs.tile_len_y*p_count < cmdLineArgs.problemSize*cmdLineArgs.problemSize\n%d < %d\n",cmdLineArgs.tile_len_x*cmdLineArgs.tile_len_y*p_count, cmdLineArgs.problemSize*cmdLineArgs.problemSize);
                   fprintf(stderr,"cmdLineArgs.tile_len_x != cmdLineArgs.problemSize\n %d != %d\n",cmdLineArgs.tile_len_x,cmdLineArgs.problemSize);
				 fprintf(stderr,"Number of processors = ceild(problem size,Y tile size). \nNumber of processors [%d] = %d. \nTry again !!\n",p_count,ceild(cmdLineArgs.problemSize,cmdLineArgs.tile_len_y));
				             }
  ...
  if((tile_y*cmdLineArgs.tile_len_y)+upperBound_y > cmdLineArgs.problemSize+1){
    upperBound_y = (cmdLineArgs.problemSize+1) - tile_y*cmdLineArgs.tile_len_y;
  }
  int tile_len_x = upperBound_x - lowerBound_x;
  int tile_len_y = upperBound_y - lowerBound_y;

 // fprintf(stderr, "Process[%d]\n\tlowerBound_x = %d\tupperBound_x = %d\n\tlowerBound_y = %d\tupperBound_y = %d\n\ttile_x = %d\ttile_y = %d\n\ttile_len_y=%d\n",rank,lowerBound_x,upperBound_x,lowerBound_y, upperBound_y, tile_x, tile_y,tile_len_y);
  ...
  for( t = 0; t < cmdLineArgs.T; ++t ){
    ...
    if ( p_count != 1) {
        MPI_Recv(&space[read][1][0], tile_len_x, MPI_DOUBLE, left, tag, MPI_COMM_WORLD,&status);
        /*MPI_Sendrecv(&space[write][upperBound_y][1], tile_len_x, MPI_DOUBLE, right, tag,
                                &space[read][upperBound_y][1], tile_len_x, MPI_DOUBLE, right, tag, MPI_COMM_WORLD,
                                &status);  
        MPI_Sendrecv(&space[write][1][1], tile_len_y, MPI_DOUBLE, left, tag,
                                &space[read][1][0], tile_len_y, MPI_DOUBLE, left, tag, MPI_COMM_WORLD,
                                &status);
        MPI_Sendrecv(&space[write][1][upperBound_y-1], tile_len_y, MPI_DOUBLE, right, tag,
                                &space[read][1][upperBound_y], tile_len_y, MPI_DOUBLE, right, tag, MPI_COMM_WORLD,
                                &status); */
    } 
    // you may find this useful for debugging
    /*for( y = lowerBound_y-1; y <= upperBound_y; ++y ){
      for( x = lowerBound_x-1; x <= upperBound_x; ++x ){
        fprintf(stderr,"%d:(%d,%d) [1] = %f\n",rank,y+(tile_len_y*rank),x,space[read][y][x]);
      }
  } */

    for( y = lowerBound_y; y < upperBound_y; ++y ){
      for( x = lowerBound_x; x < upperBound_x; ++x ){
          if ( y == 6 && x == 1 && (p_count == 1 || rank == 1) ) {
                /*printf( "\n--------------------------------------------------------\nBefore: space[%d][%d][%d] = %f\n\
                            space[%d][%d][%d] = %f\n\
                            space[%d][%d][%d] = %f\n\
                            space[%d][%d][%d] = %f\n\
                            space[%d][%d][%d] = %f\n\
                            space[%d][%d[%d] = %f\n",
                            write,y,x,space[write][y][x],
                            read,y-1,x,space[read][y-1][x],
                            read,y,x,space[read][y][x],
                            read,y+1,x,space[read][y+1][x],
                            read,y,x+1,space[read][y][x+1], 
                            read,y,x-1,space[read][y][x-1]);
                STENCIL( read, write, y, x);
               printf("After: space[write][y][x] = %f\n--------------------------------------------------------\n\n",space[write][y][x]); */
          }
          else {
            STENCIL( read, write, y, x);
          }
      }
      if ( y == lowerBound_y && rank != 0) 
            MPI_Send(&space[write][y][1], tile_len_x, MPI_DOUBLE, left, tag, MPI_COMM_WORLD); 
    }
    if ( p_count != 1 && rank != p_count - 1){
        MPI_Send(&space[write][upperBound_y-1][1], tile_len_x, MPI_DOUBLE, right, tag, MPI_COMM_WORLD);
        MPI_Recv(&space[write][upperBound_y][1], tile_len_x, MPI_DOUBLE, right, tag, MPI_COMM_WORLD, &status);
        space[write][upperBound_y-1][1] = (double)5.576494;
       /*for ( x = 0; x<=upperBound_x;x++){
        printf("%f, ",space[write][upperBound_y-1][x]);
       }
       printf("\n"); */
        /*for( x = lowerBound_x; x < upperBound_x; ++x )
            STENCIL( read, write, upperBound_y-1, x); */
    }
  /*// you may find this useful for debugging
   for( y = lowerBound_y-1; y <= upperBound_y; ++y ){
    for( x = lowerBound_x-1; x <= upperBound_x; ++x ){
      fprintf(stderr,"%d:(%d,%d) [2] = %f\n",rank,y+(tile_len_y*rank),x,space[write][y][x]);
    }
  } */
    read = write;
    write = 1 - write;
      ...
      for(pid = 1; pid<p_count; pid++){
        ...
        for( i=1; i<=tile_len_y;i++){
          ...
          MPI_Recv(&test_space[y_pos][x_pos],tile_len_x,
                   MPI_DOUBLE,pid,i,MPI_COMM_WORLD,&status);
        }
      } 
