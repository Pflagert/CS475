...
int main( int argc, char* argv[] ){
  ...
  for( t = 0; t < cmdLineArgs.T; ++t ){ 
    ...
    if ( p_count != 1 ) {
        MPI_Sendrecv(&space[write][1][1], tile_len_x, MPI_DOUBLE, left, 0,
                                &space[read][1][0], tile_len_x, MPI_DOUBLE, left, 0, MPI_COMM_WORLD,
                                &status);
        
        MPI_Sendrecv(&space[write][1][upperBound_y-1], tile_len_x, MPI_DOUBLE, right, 0,
                                &space[read][1][upperBound_y], tile_len_x, MPI_DOUBLE, right, 0, MPI_COMM_WORLD,
                                &status);
    }
