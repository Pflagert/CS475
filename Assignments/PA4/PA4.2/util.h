...
bool verifyResultJacobi2DTiled(double** result,int problemSize, int seed,int steps,
  ...
  for( x = lowerBound; x <= upperBound; ++x ){
    for( y = lowerBound; y <= upperBound; ++y ){
      if( (double)space[ steps & 1 ][x][y] != (double)result[x][y] ){
        failed = true;
        fprintf(stderr,"Position %d,%d expected: %.10lf found:%.10lf\ndiff: %.10lf\n",
                x,y,space[ steps & 1 ][x][y],result[x][y],(double)space[ steps & 1 ][x][y] - (double)result[x][y]);
        break;
      }
    }
    if( failed ) break;
