...
int main( int argc, char* argv[] ){
  ...
  for( t = 0; t < cmdLineArgs.T; ++t ){ 
    if ( rank > 0 ) {
        MPI_Recv(&space[read][0][1], tile_len_x, MPI_DOUBLE, rank - 1, 1, MPI_COMM_WORLD, &s);
        MPI_Send(&space[read][1][1], tile_len_x, MPI_DOUBLE, rank - 1, 2, MPI_COMM_WORLD);
    }
    if ( rank < p_count -1 ) {
       MPI_Sendrecv(&space[read][upperBound_y-1][1], tile_len_x, MPI_DOUBLE, rank + 1, 1,
                        &space[read][upperBound_y][1], tile_len_x, MPI_DOUBLE, rank + 1, 2, 
                        MPI_COMM_WORLD, &s);
    }
    for( y = lowerBound_y; y < upperBound_y; ++y ){
      for( x = lowerBound_x; x < upperBound_x; ++x ){
