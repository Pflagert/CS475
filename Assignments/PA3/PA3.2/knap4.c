...
void set_new_capacity(long C, long *A1, long *A2, long *max, long *cap){
    *cap = 0;
    *max = A1[0] + A2[C-0];
    for(long k=1; k<=C; k++) {
        if ( (A1[k] + A2[C-k]) > *max) {
...
void set_new_capacity_parallel(long C, long *A1, long *A2, long *max, long *cap) {
    long k, temp_max;
    int num_threads = omp_get_max_threads();
    long max_index[num_threads];
    memset(max_index, 0, sizeof(long)*num_threads);
    temp_max = A1[0] + A2[C-0];
    #pragma omp parallel for shared(max_index) firstprivate(temp_max)
    for(k=1; k<=C; k++) {
        if ( (A1[k] + A2[C-k]) > temp_max) {
            temp_max = A1[k] + A2[C-k];
    ...
    *max = -1;
    for(k=0; k<num_threads;k++){
        if ( (A1[max_index[k]] + A2[C-max_index[k]]) > *max) {
            *cap = max_index[k];
            *max = A1[*cap] + A2[C-*cap];
        }       
    }
...
long* get_last_row(long start, long end, long capacity, long *weights, long *profits) {
    ...
    cur = (long *)malloc(size);
    prev = (long *)memset(malloc(size), 0, size);
    
    for(i=start; i<=end; i++){
        #pragma omp parallel for if(capacity >= 80000)
            for(j=0;j<=capacity;j++){
                    cur[j] = (j < weights[i]) ? prev[j] : MAX(prev[j], (prev[j-weights[i]] + profits[i]) );
            }
        temp = cur;
        cur = prev;
        prev = temp;
...
long solve_kp(long i, long j, long C, long *weights, long *profits, int *solutions) {
        ...
        if(C >= 80000)
            set_new_capacity_parallel(C, A1, A2, &max, &cap);
        else
            set_new_capacity(C, A1, A2, &max, &cap);
        
        free(A1);
        free(A2);
        
        if (sec1 = solve_kp_base(i, i+m, cap, weights, profits, solutions) == 0)
            #pragma omp task final(cap < 80000)
                sec1 =  solve_kp(i, i+m, cap, weights, profits, solutions);
        sec1 = MAX(max,sec1);
        
        if(sec2 = solve_kp_base(i+m+1, j, C-cap, weights, profits, solutions) == 0)
            #pragma omp task final(C-cap < 80000)
                sec2 = solve_kp(i+m+1, j, C-cap, weights, profits, solutions);
        sec2 = MAX(max,sec2);
...
int main(int argc, char **argv) {
    ...
    long final = solve_kp(0,N-1, C, weights, profits, solution);
    stop_timer();
    time = elapsed_time ();
