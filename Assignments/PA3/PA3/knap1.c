...
long* get_last_row(long start, long end, long N, long capacity, long *weights, long *profits) {
    long i,j;
    long *cur, *prev, *temp;
    cur = (long *)malloc(sizeof(long)* (capacity+1));
    prev = (long *)malloc(sizeof(long) * (capacity+1));
...
long solve_kp_base(long i, long j, long N, long C, long *weights, long *profits, int *solutions) {
    long profit = 0;
    if (C <= 0){
      return -1;  
    }
...
long solve_kp(long i, long j, long N, long C, long *weights, long *profits, int *solutions) {
        long k, cap, max, sec1, sec2;
        long *A1, *A2;
        long m = ((j-i)/2) - (j%2);
        
        A1 = get_last_row(i, i+m, N, C, weights, profits);
        A2 = get_last_row(i+m+1, j, N, C, weights, profits);
        ...
        free(A1);
        free(A2);
        if (sec1 = solve_kp_base(i, i+m, N, cap, weights, profits, solutions) == 0)
                sec1 =  solve_kp(i, i+m, N, cap, weights, profits, solutions);
        sec1 = MAX(max,sec1);
       
        if(sec2 = solve_kp_base(i+m+1, j, N, C-cap, weights, profits, solutions) == 0)
            sec2 = solve_kp(i+m+1, j, N, C-cap, weights, profits, solutions);
        sec2 = MAX(max,sec2);
...
int main(int argc, char **argv) {
    ...
    long final = solve_kp(0,N-1,N, C, weights, profits, solution);
    stop_timer();
    time = elapsed_time ();
