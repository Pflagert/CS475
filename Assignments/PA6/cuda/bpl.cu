...
void forwardPhase(long b) {
    mm(Zh,X,Wxh, b, cmdLineArgs.M, b, cmdLineArgs.N+1, cmdLineArgs.N+1, cmdLineArgs.M,device_Zh,device_X,device_Wxh); //Zh=X*Wxh
    //func_old(H,Zh,b,cmdLineArgs.M,1); //H=f1(Zh)
    func(H,Zh,b,cmdLineArgs.M+1, b, cmdLineArgs.M, 1, device_H, device_Zh);
    
    mm(Zy,H,Why, b, cmdLineArgs.P, b, cmdLineArgs.M+1, cmdLineArgs.M+1, cmdLineArgs.P,device_Zy,device_H,device_Why); //Zy=H*Why	
    
    //func_old(P,Zy,b,cmdLineArgs.P,0); //P=fn(Zy)	
    func(P,Zy,b,cmdLineArgs.P,b,cmdLineArgs.P,0,device_P,device_Zy);
    
    //reduction_old(P,sum,b,cmdLineArgs.P);
    reduction(P,sum,b,cmdLineArgs.P,b,device_P,device_Sum);  //summation of probabilities for each training sample
    //prob_old(P,P1,sum,b,cmdLineArgs.P); //P1=fn(P,sum)	
    prob(P,P1,sum, b, cmdLineArgs.P, b, cmdLineArgs.P, b, device_P,device_P1,device_Sum);
    //error_old(E,P1,Y,b,cmdLineArgs.P);	//E=P1-Y
    error(E,P1,Y,b,cmdLineArgs.P,b,cmdLineArgs.P,b,cmdLineArgs.P,device_E,device_P1);
}

void propagationPhase(long b) {
    //mtm_old(dWhy,H,E,cmdLineArgs.M+1,b,cmdLineArgs.P); //dWhy=H'*E ('->transpose)	
    mtm(dWhy,H,E,cmdLineArgs.M+1,cmdLineArgs.P,b,cmdLineArgs.M+1,b,cmdLineArgs.P,device_dWhy,device_H,device_E); //dWhy=H'*E ('->transpose
    //delta_old(Why,dWhy,cmdLineArgs.M+1,cmdLineArgs.P,learningrate); //Why=fn(dwhy)                          *
    delta(Why,dWhy,cmdLineArgs.M+1,cmdLineArgs.P,cmdLineArgs.M+1,cmdLineArgs.P,learningrate,device_Why,device_dWhy); 
     //mmt_old(H,Why,E,b,cmdLineArgs.M+1,cmdLineArgs.P);
    mmt(H,Why,E,b,cmdLineArgs.M+1,cmdLineArgs.M+1,cmdLineArgs.P,b,cmdLineArgs.P,device_H,device_Why,device_E); //H=Why*E'	
     //gradient_func_old(Zh,H,b,cmdLineArgs.M); //Zh=f1"(H) ("->gradient of f1)	
     gradient_func(Zh,H,b,cmdLineArgs.M,b,cmdLineArgs.M+1,device_Zh,device_H); //Zh=f1"(H) ("->gradient of f1)		
     
     //mtm_old(dWxh,X,Zh,cmdLineArgs.N+1,b,cmdLineArgs.M); //dWhy=H'*E ('->transpose)	
     mtm(dWxh,X,Zh,cmdLineArgs.N+1,cmdLineArgs.M,b,cmdLineArgs.N+1,b,cmdLineArgs.M,device_dWxh,device_X,device_Zh);	//dWxh=X'Zh
     
     //delta_old(Wxh,dWxh,cmdLineArgs.N+1,cmdLineArgs.M,learningrate);//Wxh=fn(dWxh)
     delta(Wxh,dWxh,cmdLineArgs.N+1,cmdLineArgs.M,cmdLineArgs.N+1,cmdLineArgs.M,learningrate,device_Wxh,device_dWxh);//Wxh=fn(dWxh) */
}
